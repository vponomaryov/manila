...
function scenario_1_do_share_with_rules_and_metadata {
    ...
    create_share_cmd="manila create $share_protocol 1 --share-type $MANILA_GRENADE_SHARE_NAME"
    ...
    if [[ $(trueorfalse False driver_handles_share_servers) == True ]]; then
        ...
        share_network_cmd="manila share-network-create --name $MANILA_GRENADE_SHARE_NETWORK_NAME"
    ...
    manila metadata myshare set gre=nade

    # Add access rules
    manila access-allow myshare $MANILA_GRENADE_ACCESS_TYPE $MANILA_GRENADE_ACCESS_TO
...
function scenario_1_verify_share_with_rules_and_metadata {
    ...
    rule_state=$(manila access-list $MANILA_GRENADE_SHARE_NAME | \
        grep " $MANILA_GRENADE_ACCESS_TO " | cut -d " " -f 10)
...
function scenario_1_destroy_share_with_rules_and_metadata {
    ...
    while (( $timeout > 0 )) ; do
        share_status=$( manila list --columns id,name,status | \
            grep $MANILA_GRENADE_SHARE_NAME | cut -d " " -f 6)
...
function scenario_2_do_attach_ss_to_sn {
    ...
    manila share-network-create \
        ...
        --neutron-network fake_net \
        --neutron-subnet fake_subnet
...
function scenario_2_verify_attach_ss_to_sn {
    ...
    function assert {
        actual=$(manila $1 $2 | grep " $3 " | cut -d " " -f 4)
        if [[ $actual != $4 ]]; then
...
function scenario_4_do_private_share_types {
    manila type-create p --is-public false false
    manila type-access-add p $(openstack project show demo -c id -f value)
}

function scenario_4_verify_private_share_types {
    share_type_visibility=$(manila type-list --all --columns visibility | \
        grep p | cut -d " " -f 2)
    if [[ $share_type_visibility != 'private' ]] ; then
        die $LINENO "Visibility of share type p is not 'private'. "\
                    "It is $share_type_visibility"
    fi

    project_id=$(openstack project show demo -c id -f value)
    access=$(manila type-access-list p | grep $project_id)
    if [[ -z $access ]]; then
        die $LINENO "Expected $project_id project ID is not found in list "\
                    "of allowed projects of p share type."
    fi
}

function scenario_4_destroy_private_share_types {
    manila type-delete p
}
