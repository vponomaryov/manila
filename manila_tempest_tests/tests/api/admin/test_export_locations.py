...
class ExportLocationsTest(base.BaseSharesAdminTest):
    ...
    def _verify_export_location_structure(self, export_locations,
                                          role='admin'):
        expected_keys = [
            'created_at', 'updated_at', 'path', 'uuid', 'metadata']
        if role == 'admin':
        ...
        for export_location in export_locations:
            ...
            self.assertTrue(isinstance(export_location['metadata'], dict))
            self.assertTrue(
    ...
    @test.attr(type=["gate", ])
    def test_get_share_export_location_metadata(self):
        export_locations = self.admin_client.list_share_export_locations(
            self.share['id'])
        for el in export_locations:
            metadata = self.admin_client.get_share_export_location_metadata(
                self.share['id'], el['uuid'])
    ...
    @test.attr(type=["gate", ])
    def test_get_share_export_location_metadata_by_member(self):
        export_locations = self.member_client.list_share_export_locations(
            self.share['id'])
        for el in export_locations:
            metadata = self.member_client.get_share_export_location_metadata(
                self.share['id'], el['uuid'])
            self.assertTrue(isinstance(metadata, dict))
    ...
    @test.attr(type=["gate", ])
    def test_get_share_instance_export_location_metadata(self):
        for share_instance in self.share_instances:
            export_locations = (
            ...
            for el in export_locations:
                m = self.admin_client.\
                    get_share_instance_export_location_metadata(
                        share_instance['id'], el['uuid'])
