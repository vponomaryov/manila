...
class ExportLocationsNegativeTest(base.BaseSharesAdminTest):
    ...
    @test.attr(type=["gate", "negative"])
    def test_get_inexistent_share_export_location_metadata(self):
        self.assertRaises(
            lib_exc.NotFound,
            self.admin_client.get_share_export_location_metadata,
            self.share['id'],
            "fake-inexistent-share-instance-id",
        )
    ...
    @test.attr(type=["gate", "negative"])
    def test_get_inexistent_share_instance_export_location_metadata(self):
        for share_instance in self.share_instances:
            self.assertRaises(
                lib_exc.NotFound,
                self.admin_client.get_share_instance_export_location_metadata,
                share_instance['id'],
                "fake-inexistent-share-instance-id",
            )
    ...
    @test.attr(type=["gate", "negative"])
    def test_get_share_instance_export_location_metadata_by_member(self):
        for share_instance in self.share_instances:
            export_locations = (
                self.admin_client.list_share_instance_export_locations(
                    share_instance['id']))
