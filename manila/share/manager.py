...
class ShareManager(manager.SchedulerDependentManager):
    ...
    def create_replicated_snapshot(self, context, snapshot_id, share_id=None):
        ...
        replica_list = [self._get_share_replica_dict(context, r)
                        for r in replica_list]
        snapshot_instances = [self._get_snapshot_instance_dict(context, s)
                              for s in snapshot_instances]
        updated_instances = []
    ...
    def delete_replicated_snapshot(self, context, snapshot_id,
        ...
        try:
            ...
            with excutils.save_and_reraise_exception() as e:
                if force:
                    deleted_instances = snapshot_instances
    ...
    def periodic_share_replica_snapshot_update(self, context):
        ...
        synced_host_replicas = list(filter(
            lambda x: qualified_replica(x), replicas))
        transitional_snapshot_instances = []

        for replica in synced_host_replicas:
            filters = {
                'share_instance_ids': replica['id'],
                'statuses': transitional_statuses,
            }
            replica_snapshot_instances = (
                self.db.share_snapshot_instance_get_all_with_filters(
                    context, filters, with_share_data=True)
            )
            transitional_snapshot_instances.extend(replica_snapshot_instances)

        for snapshot_instance in transitional_snapshot_instances:
            snapshot_instances = (
                self.db.share_snapshot_instance_get_all_with_filters(
                    context,
                    {'snapshot_ids': snapshot_instance['snapshot_id']})
            )
            share_id = snapshot_instance['share']['share_id']
            self._update_replica_snapshot(
                context, snapshot_instance,
                snapshot_instances=snapshot_instances, share_id=share_id)

    @locked_share_replica_operation
    def _update_replica_snapshot(self, context, snapshot_instance,
                                 snapshot_instances=None, share_id=None):
        # Re-grab the replica:
        try:
            share_replica = self.db.share_replica_get(
                ...
                with_share_data=True, with_share_server=True)
            snapshot_instance = self.db.share_snapshot_instance_get(
                context, snapshot_instance['id'], with_share_data=True)
        ...
        msg_payload = {
            'snapshot_instance': snapshot_instance['id'],
            'replica': share_replica['id'],
        }

        LOG.debug("Updating status of share snapshot %(snapshot_instance)s: "
        ...
        replica_list = [self._get_share_replica_dict(context, r)
                        for r in replica_list]
        snapshot_instances = snapshot_instances or []
        snapshot_instances = [self._get_snapshot_instance_dict(context, s)
                              for s in snapshot_instances]
        ...
        try:
            ...
            snapshot_update = self.driver.update_replicated_snapshot(
                context, replica_list, share_replica, snapshot_instances,
                snapshot_instance, share_server=share_server) or {}

        except exception.NotFound:
            with excutils.save_and_reraise_exception() as e:
                if snapshot_instance['status'] == constants.STATUS_DELETING:
        ...
        if snapshot_update:
            ...
            self.db.share_snapshot_instance_update(
                context, snapshot_instance['id'], snapshot_update)
