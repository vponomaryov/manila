...
class ShareExportLocationsAPITest(test.TestCase):
    ...
    def test_list_and_show(self, role):
        ...
        for index_el in index_result['export_locations']:
            ...
            expected_keys = [
                'created_at', 'updated_at', 'uuid', 'path', 'metadata']
            if role == 'admin':
    ...
    def test_get_metadata(self):
        ...
        metadata = self.controller.get_metadata(
            self.req, self.share['id'], el_uuid)
        ...
        db.export_location_metadata_update(
            self.ctxt['admin'], el_uuid, expected_metadata, False)

        metadata = self.controller.get_metadata(
            self.req, self.share['id'], el_uuid)

        self.assertEqual(expected_metadata, metadata.get('metadata', 'fake'))
    ...
    def test_get_metadata_share_not_found(self):
        index_result = self.controller.index(self.req, self.share['id'])
    ...
    def test_get_metadata_export_location_not_found(self):
        self.assertRaises(
            exc.HTTPNotFound,
            self.controller.get_metadata,
            self.req, self.share['id'], 'inexistent_export_location',
    ...
    def test_get_admin_export_location_metadata(self):
        el_data = {
            'path': '/admin/export/location',
            'is_admin_only': True,
            'metadata': {'foo': 'bar'},
        ...
        metadata = self.controller.get_metadata(
            self.req, self.share['id'], el_uuid)
        self.assertEqual(el_data['metadata'], metadata.get('metadata'))
    ...
    @ddt.data('1.0', '2.0', '2.8')
    def test_get_metadata_with_unsupported_version(self, version):
        index_result = self.controller.index(self.req, self.share['id'])
