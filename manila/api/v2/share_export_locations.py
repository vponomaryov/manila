...
class ShareExportLocationController(wsgi.Controller):
    ...
    @wsgi.Controller.api_version('2.9')
    @wsgi.Controller.authorize
    def get_metadata(self, req, share_id, export_location_uuid):
        """Return metadata for the requested export location."""
        context = req.environ['manila.context']
        self._verify_share(context, share_id)
        not_found_msg = _(
            "Export location '%s' not found.") % export_location_uuid
        ...
        try:
            el = db_api.share_export_location_get_by_uuid(
                context, export_location_uuid)
            metadata = db_api.export_location_metadata_get(
                context, export_location_uuid)
        except exception.ExportLocationNotFound:
            raise exc.HTTPNotFound(explanation=not_found_msg)

        if context.is_admin or not el.is_admin_only:
