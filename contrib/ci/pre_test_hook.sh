...
if [[ "$DRIVER" == "generic" ]]; then
    ...
    echo "SHARE_DRIVER=manila.tests.share.drivers.dummy.DummyDriver" >> $localrc_path
    echo "MANILA_ENABLED_BACKENDS=alpha,beta" >> $localrc_path
    ...
    echo "MANILA_OPTGROUP_gamma_replication_domain=GAMMA" >> $localrc_path
    ...
    echo "MANILA_OPTGROUP_delta_replication_domain=DELTA" >> $localrc_path
    ...
    export TEMPEST_CONCURRENCY=24
    echo "TEMPEST_CONCURRENCY=$TEMPEST_CONCURRENCY" >> $localrc_path
...
echo "TEMPEST_CONCURRENCY={TEMPEST_CONCURRENCY:-8}" >> $localrc_path
